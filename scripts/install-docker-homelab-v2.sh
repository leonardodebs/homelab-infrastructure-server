...
# Evita validação frágil com pipeline + grep -q sob `set -o pipefail`.
# Capturamos a saída primeiro e validamos em memória.
DOCKER_POLICY="$(apt-cache policy docker-ce)"
printf '%s\n' "$DOCKER_POLICY"

CANDIDATE="$(awk '/Candidate:/ {print $2; exit}' <<< "$DOCKER_POLICY")"
[[ -n "$CANDIDATE" && "$CANDIDATE" != "(none)" ]] \
    || fail "Nenhuma versão candidata de docker-ce encontrada."

DOCKER_MADISON="$(apt-cache madison docker-ce || true)"
printf '%s\n' "$DOCKER_MADISON"
if [[ "$DOCKER_MADISON" != *"download.docker.com"* ]]; then
    fail "docker-ce tem candidato, mas a origem oficial download.docker.com não foi confirmada."
fi
