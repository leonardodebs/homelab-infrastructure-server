...
grep -q -- '-A ufw-user-input -p tcp --dport 22 -s 192.168.15.0/24 -j ACCEPT' \
...
readonly PROMETHEUS_IP="192.168.15.3/32"
readonly LEGACY_SUBNET="192.168.100.0/24"
...
"$IPTABLES" -C DOCKER-USER -s "$LEGACY_SUBNET" -j DROP 2>/dev/null \
  || "$IPTABLES" -I DOCKER-USER 1 -s "$LEGACY_SUBNET" -j DROP
...
for port in 8001 9100; do
  iptables -C DOCKER-USER -s 192.168.15.3/32 -p tcp --dport "$port" -j ACCEPT
  iptables -C DOCKER-USER -p tcp --dport "$port" -j DROP
done
iptables -C DOCKER-USER -s 192.168.15.3/32 -p tcp -m conntrack --ctdir ORIGINAL --ctorigdstport 8081 -j ACCEPT
