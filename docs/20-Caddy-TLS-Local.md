...
O Caddy roda em `network_mode: host` (mesmo modelo do AdGuard) e assumiu as portas públicas que antes pertenciam diretamente a cada serviço. Cada serviço migrou para uma porta interna, ainda vinculada a `192.168.15.2` (não `127.0.0.1`), porque o Uptime Kuma monitora esses serviços pela rede Docker e precisa continuar alcançando-os por IP.

| Serviço | Porta pública HTTPS (Caddy) | Porta interna real |
|---|---|---|
| AdGuard Home | `8443` (a `80` continua HTTP puro — ver nota abaixo) | `192.168.15.2:8280` |
| ntopng | `3000` | `192.168.15.2:3300` |
| Uptime Kuma | `3001` | `192.168.15.2:3101` |
...
- AdGuard Home respondendo em `192.168.15.2:53`;
...
- **AdGuard Home**: editar `http.address` em `AdGuardHome.yaml` de `192.168.15.2:80` para `192.168.15.2:8280` — **exige parar o container por alguns segundos** (mesmo processo que serve DNS), então guarde o túnel SSH de emergência antes:
  ```bash
  ...
  sudo sed -i 's/^  address: 192.168.15.2:80$/  address: 192.168.15.2:8280/' "$CONF"
  docker start adguardhome
  dig @192.168.15.2 cloudflare.com
  ```
...
```bash
sudo ufw allow from 172.18.0.0/16 to 192.168.15.2 port 8280 proto tcp comment 'Docker monitor AdGuard Web'
sudo ufw allow from 172.18.0.0/16 to 192.168.15.2 port 8180 proto tcp comment 'Docker monitor HomeLab Web'
sudo ufw allow from 172.18.0.0/16 to 192.168.15.2 port 9444 proto tcp comment 'Docker monitor Portainer'
sudo ufw delete allow from 172.18.0.0/16 to 192.168.15.2 port 80 proto tcp
sudo ufw delete allow from 172.18.0.0/16 to 192.168.15.2 port 8080 proto tcp
sudo ufw delete allow from 172.18.0.0/16 to 192.168.15.2 port 9443 proto tcp
```
...
| Monitor | URL nova |
|---|---|
| AdGuard Web | `http://192.168.15.2:8280` |
| Portainer | `https://192.168.15.2:9444` |
| HomeLab Web | `http://192.168.15.2:8180` |

## 6. Cadastrar os nomes locais no AdGuard Home

Em **Filters → DNS rewrites**, adicione, todos apontando para `192.168.15.2`:
...
Copie `caddy-root-ca.crt` para o Windows (ex.: `scp leonardo@192.168.15.2:~/homelab-infrastructure-server/caddy-root-ca.crt .`) e instale como raiz confiável **em uma sessão elevada** (o Windows exige confirmação por interface gráfica; não dá pra automatizar):
...
- [x] `https://192.168.15.2:8443` (AdGuard), `:3000`, `:3001`, `:8080`, `:9443` abrem sem aviso de certificado depois de instalar a CA;
- [x] `http://192.168.15.2` (porta 80) continua respondendo em HTTP puro, sem regressão;
